...
/**
  Plugin Name: Woo Metaphone Product Search 
  Plugin URI:
...
function process_gc_doublemetaphone() {
    ...
    if (isset($_POST['gc_updatemetavalues']) && sanitize_text_field($_POST['gc_updatemetavalues']) != '') {
...
function gc_dm_search_products() {
    ...
    if (defined('DOING_AJAX') && DOING_AJAX) {
        $searchkey = sanitize_text_field(trim(urldecode($_REQUEST['key'])));
        
        if ($_REQUEST['key'] != NULL && strlen($searchkey) >= get_option('gc_num_chars')) {
            $process->search_products($searchkey);
        }
    }
    exit();
}
...
function gc_doublemetaphone_validate_yes_no($value){
