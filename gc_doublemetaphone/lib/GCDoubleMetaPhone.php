...
//        $this->_message .= '<p>Total failed conversions: ' . $this->_failedCount . '</p>';
    ...
    public function search_products($searchkey) {
        global $wpdb;
        //security
        $searchkey = esc_sql($searchkey);
        $items = explode(' ', $searchkey);
        ...
        $sql = "select distinct dm.woo_product_id,dm.product_image_url,dm.name_txt, MATCH (dm.name_metaphone,dm.short_des_metaphone,des_metaphone) "
                ...
                . "ORDER by (CAST(Relevanz AS SIGNED) + (6*CAST(RelevanzName AS SIGNED)) + (4*CAST(RelevanzPhNome AS SIGNED)) + (3*CAST(Relevanz2 AS SIGNED))) DESC, dm.name_txt ASC LIMIT " . get_option('gc_num_results');
    ...
    public function prepareSearchHtmlRes() {
        ...
        if (sizeof($this->_searchRes) > 0) {
            ...
            foreach ($this->_searchRes as $res) {
                $html .= '<tr>';
                $html .= '<td class="prod_img"><img src="' . esc_html($res->product_image_url) . '"></td>';
                $html .= '<td><a href="' . get_permalink($res->woo_product_id) . '">' . esc_html($res->name_txt) . '</a></td>';
                $html .= '</tr>';
            }
            $html .= '</tbody></table>';
        }
